        ...
        with col_grid:
            ...
            with st.container(height=600):
                ...
                for i, card in enumerate(ana_available):
                    with grid_cols[i % 4]:
                        if st.button(card, key=f"ana_btn_{card}"):
                ...
                if not synergy_df.empty and card in synergy_df.columns:
                    ...
                    for idx, s_card in enumerate(top_syns):
                        with s_cols[idx % 5]:
                            ...
                            st.markdown(f"<div style='font-size:10px; text-align:center;'>{s_card}</div>", unsafe_allow_html=True)
                    ...
                    for idx, c_card in enumerate(top_cnts):
                        with c_cols[idx % 5]:
                            ...
                            st.markdown(f"<div style='font-size:10px; text-align:center;'>{c_card}</div>", unsafe_allow_html=True)
                else:
